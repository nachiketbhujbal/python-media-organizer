...
The mode revalidates the unchanged private policy, strict lifecycle, typed
outcomes, version and options binding, and both collection identities between
children and immediately before each authorized transition. Every checkpoint
has its own exact expected aggregate; one authorization never covers another.
