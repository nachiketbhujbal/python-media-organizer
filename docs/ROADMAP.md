...
| 0.6.3 | Saved invocation context and resume | Persist the already-private run binding for exact roots, log location, options, and current checkpoint so an interrupted operator can resume without reconstructing the long original command. Restart bookkeeping remains non-evidence. | Release candidate |
