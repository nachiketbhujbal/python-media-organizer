...
| POLICY-R05 | High | An existing group- or world-writable log directory let another user replace both the restart state and supposedly create-once binding record, allowing substituted authority to appear internally consistent. | 0.6.5 | Corrected in the current candidate by requiring an owner-private log directory and non-writable ancestry, with only ownership-safe sticky ancestors accepted, before lock creation or unattended state access; regressions reject an initially public directory without writing and reject jointly replaced authority after the directory becomes public. Interrupted first-state publication now recovers only from the exact durable binding; renewed independent acceptance is pending |
