...
| POLICY-R03 | High | The first policy-binding correction omitted the persisted digest from the active operator-binding comparison, so a schema-valid state substitution on a failing child path could replace the digest and let a later policy inherit the run. | 0.6.5 | Resolved after renewed independent review by including the policy digest in every operator-binding comparison, rechecking full policy binding after every child result, and proving a failure-path digest substitution is rejected while a later replacement-policy resume dispatches no child |
