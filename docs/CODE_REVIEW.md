...
| GUIDE-R06 | Medium | Interactive refusal of a pending status-one validation initially returned 0, hiding the recorded child's real finding status behind an ordinary pause. | 0.6.1 | Resolved after independent review by returning the recorded status 1 until explicit acknowledgement and proving negative or empty input leaves restart state byte-identical |
| GUIDE-R07 | Medium | The first v0.6.1 candidate marked the release as complete before merge, exact-main CI, and tag evidence existed. | 0.6.1 | Resolved after independent review by keeping candidate documentation unreleased until the tag boundary was proven, then reconciling current `main` through the separate post-tag process in ADR 0090 |
| GUIDE-011 | High | Re-parsing human stage logs or repeating expensive analysis for a final synopsis could mistake display text for evidence, couple compatibility to prose, or summarize a later filesystem state. | 0.6.2 | Resolved with strict aggregate child-owned outcome records validated against the exact stage, command, result kind, exit status, and private restart lifecycle; the synopsis performs no media analysis |
| GUIDE-012 | High | A concise report could collapse preview into mutation, simulation into observed proof, potential duplicate storage into reclaimed capacity, or coordinator bookkeeping into deletion authority. | 0.6.2 | Resolved with explicit preview-versus-isolated, simulated-versus-observed, pending-versus-complete, and potential-versus-proven language plus an unconditional non-evidence and no-delete scope statement |
| GUIDE-013 | Medium | Adding private synopsis records to every child command could change standalone behavior or allow malformed, linked, public, nested, mismatched, or substituted outcome files to advance the coordinator. | 0.6.2 | Resolved with an immediate standalone no-outcome path, private no-replace creation outside both collections, descriptor-pinned strict reads, exact schema and accounting checks, and fail-closed coordinator advancement |
| STATE-R01 | High | Independent review proved a resumed coordinator could dispatch the next child after a prior successful outcome reference was removed or made unsafe, because lifecycle parsing allowed a null reference and history was validated only while printing the later synopsis. | 0.6.2 | Resolved by requiring an outcome reference for every status-0 child and reviewed status-1 validation, validating the complete referenced history before every resume action and child dispatch, and proving an unsafe prior outcome leaves the next stage undispatched and state unchanged |
...
| GUIDE-020 | High | A general unattended “yes to all” switch could silently accept changed validation findings or a different mutation plan than the operator intended. | 0.6.5 | Resolved by requiring one ordered authorization per checkpoint, exact aggregate matching, and a versioned digest of each ordered private source/target plan against the validated typed outcome immediately before its transition |
| GUIDE-021 | High | A substituted, edited, linked, public, collection-local, oversized, or version-mismatched policy could grant authority the operator did not supply to the bound run. | 0.6.5 | Resolved with stable no-follow private-file reads, exact byte and identity rechecks, one-link and size limits, collection exclusion, strict schema fields, and exact version/root/options binding before state creation and throughout execution |
| GUIDE-022 | High | Pre-authorization could bypass the preview-before-apply engine, turn simulated preservation into final proof, or treat duplicate review as permission to move or delete content. | 0.6.5 | Resolved by matching applies only to their successful preview outcomes, matching quarantine only to complete simulated evidence while retaining the absent-`dups` human-managed checkpoint, and matching sign-off only to complete ordinary final verification |
| GUIDE-023 | Medium | An unattended loop could hide an unexpected child result or continue after state, outcome, collection identity, or policy changed between stages. | 0.6.5 | Resolved by returning child statuses unchanged, validating every exact transition and complete outcome history, and rechecking saved binding, collection identities, and policy stability before later dispatch or acknowledgement |
| DIGEST-R01 | High | The first v0.6.5 candidate bound organization and rename apply decisions only to paths and derived metadata, so different content substituted at the same path could inherit the reviewed authorization. | 0.6.5 | Resolved after independent review by hashing every planned source through a stable collection-anchored descriptor, including its SHA-256 in the decision digest, carrying its exact state and hash into journal action construction, and rejecting same-path replacement before mutation in both commands |
| POLICY-R02 | Medium | A resumed run could replace its first valid policy with a different valid policy because restart state did not retain policy provenance despite promising the same unchanged policy. | 0.6.5 | Resolved after independent review by binding the first policy payload SHA-256 into private restart state, requiring the identical digest on every unattended resume, and proving a broader substitute policy dispatches no child and leaves bound state unchanged |
...
Renewed independent review accepted the final review-ledger-only v0.6.6 owner
head `00c852264f0aa89c4e9bbc6d111413f9cb7c7278` without findings and confirmed
that it accurately records the earlier exact-SHA reviews. PR #58 full-platform
run `34711419627` passed at that exact head. The protected merge is
`75bf41788297fc48570952d675f4af3e1d9ce8dd`; exact-main run `34711660008`
passed before annotated tag `v0.6.6` was created; and tag-triggered release run
`34711926429` passed the mainline, build, tagged-version, and isolated-install
boundaries. A separate tagged build and installed-command check reproduced
...
| MIG-R02 | Medium | Standalone `verify-migration` resolves both command-line roots before its controlled identity-error boundary, so a self-referential symbolic link or another resolution failure can escape as status 1 with a traceback and absolute-path disclosure. | 0.5.12 | Resolved by catching `OSError` and `RuntimeError` for both root resolutions and adding real source/destination symbolic-link-cycle plus symmetric injected-error privacy and zero-write regressions; independent review accepted exact head `be36189` without findings |
| MIG-R03 | Medium | The shared required-root comparison discarded the initial endpoint identities and reused a permissive ancestry helper, so a root that disappeared or changed identity during a later observation could be treated as merely non-ancestral and allow a false complete migration result. | 0.5.12 | Resolved by retaining and comparing both initial identities, using a strict existing-root ancestry walk, and revalidating both endpoints after both walks; real disappearance and replacement races cover each endpoint during ancestry and final revalidation, and independent review accepted exact head `be36189` without findings |

## Release-documentation review findings
...
| DOC-010 | High | Initial ADR 0108 proposed exporting complete target authority from a reviewed run at a different root while also forbidding authority transfer across changed roots and acknowledging that later mutation-dependent evidence cannot come from a pure preview, leaving the promised one-command path unsatisfiable. | 0.7.1 roadmap | Resolved by requiring one complete target-local, zero-media-mutation plan that composes deterministic namespace transitions, derives every later expectation from the target's descriptor-pinned bytes, stops when a stage cannot be modeled safely, and remains non-authoritative until separately accepted. |
