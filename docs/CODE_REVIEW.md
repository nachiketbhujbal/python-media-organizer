...
| POLICY-R04 | High | The POLICY-R03 regression did not rewrite persisted restart state, leaving that replaceable file as the sole provenance for a later process and allowing a substituted digest plus replacement policy to inherit the run. | 0.6.5 | Corrected in the current candidate by creating a separate no-replace private binding record for the original policy and run creation fields, cross-checking it at every unattended boundary, and rewriting the real state file in the regression before proving both the current run and later replacement-policy resume stop without another child dispatch; renewed independent acceptance is pending |
