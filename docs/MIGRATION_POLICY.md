...
directory ownership rules protect the next component. Every component in the
ancestry must be owned by root or the current user, so an untrusted owner cannot
make a previously non-writable parent replaceable later. This requirement is
