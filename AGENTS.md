...
- A meaningful non-media content signature outranks a media extension. Never
  ...
  mismatch, invoke no decoder or native tool for it, and count it as a non-media
  file. An empty stream is the absence of a signature rather than a non-media
  one, so an empty media file is still validated and still an error.
